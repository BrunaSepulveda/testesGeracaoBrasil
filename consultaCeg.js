...
async function html() {
  ...
  quantidade = Number(quantidade.replace(/(?<=\s+.*)\s+/g, '').replace('Quantidade:','').replace('.','').trim())
  ...
  function extrairDados (response) {
    ...
    .each((i, linha) => {
      ...
      if( Object.keys(linhaObj).length > 0) {
        const obj = { ceg: '', tipo: '', situacao: '',nome: '', razao: '',cnpj: '',municipio: '',uf: '',data: '', garatiaFisica: ''};
        ...
        const razaoCnpj = linhaObj['Proprietário'] ? formatter (linhaObj['Proprietário']): null;
        const municipioUf = linhaObj['Município'] ? formatter(linhaObj['Município']): null;
        obj.razao = razaoCnpj && razaoCnpj.length > 0 ? razaoCnpj[0] : null;
        obj.cnpj = razaoCnpj && razaoCnpj.length > 0 ? razaoCnpj[1] : null;
        obj.municipio = municipioUf && municipioUf.length > 0 ? municipioUf[0] : null;
        obj.uf = municipioUf && municipioUf.length > 0 ? municipioUf[1] : null;
        corpo.push(obj)
      }
    })
    return corpo;
